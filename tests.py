...
for n in sizes:
    ...
    for k, v in datas.items():
        ...
        result = check_distributions(v, ALL_DISTRIBUTIONS)
        ...
        result["Dataframe"].to_csv(os.path.join(res_path, f"{k} Distribution ({n} samples) Results.csv"), index=False)
        # Enregistrement du Rapport Complet
        make_distribution_report(v, result, f"{k} Distribution ({n} samples) Report", res_path)

        print(f"  Vérification de normalité avec ses versions transformées")
        # Vérification de normalité avec ses versions transformées
        result = check_normality(transform(v))
        # Enregistrement de la figure
        fig = result["Figure"]
        fig.tight_layout()
        fig.savefig(os.path.join(res_path, f"{k} Distribution ({n} samples) Normality Histogramm.png"), bbox_inches="tight")
        # Enregistrement du tableau récapitualtif
        result["Dataframe"].to_csv(os.path.join(res_path, f"{k} Distribution ({n} samples) Normality Results.csv"), index=False)
        # Enregistrement du Rapport Complet
        make_normality_report(v, result, f"{k} Distribution ({n} samples) Normality Report", res_path)
